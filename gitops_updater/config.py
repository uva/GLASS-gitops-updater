...
class ConfigReader:
    def find(self, filename: str, name: str) -> ConfigEntry:
        ...
        raise Exception(
            f"Couldn't read config: Unable to locate entry with the name '{name}'"
        )
                ...
                if row["name"] == name:
                    if row["type"] == "GitHub":
                        return GitHubProvider(
                            row["tokenPath"], row["branch"], row["repository"]
                        )
                    if row["type"] == "GitLab":
                        return GitLabProvider(
                            row["url"], row["tokenPath"], row["branch"], row["project"]
                        )

                    raise Exception(
                        f"Couldn't read config: Unable to resolve provider type '{row['type']}'"
                    )

        raise Exception(
            f"Couldn't read config: The provider with name '{name}' could not be found"
        )
